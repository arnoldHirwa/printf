	...
	int flags, int width, int precision, int size)
{
	...
	if (is_negative)
		extra_ch = '-';
	else if (flags & V_PLUS)
		extra_ch = '+';
	else if (flags & V_SPACE)
		extra_ch = ' ';

	return (handle_num(ind, buffer, flags, width, precision,
		length, padd, extra_ch));
}

/**
 * handle_num - Write a number using a bufffer
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & V_MINUS && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[ind], length) + write(1, &buffer[1], i - 1));
		}
		else if (!(flags & V_MINUS) && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], i - 1) + write(1, &buffer[ind], length));
		}
		else if (!(flags & V_MINUS) && padd == '0')
		{
			if (extra_c)
...
/**
 * handle_unSign - Writes an unsigned number
